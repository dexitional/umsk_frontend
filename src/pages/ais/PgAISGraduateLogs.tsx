...
import GraduateLogCardItem from "../../components/ais/GraduateLogCardItem";
...
export async function loader({ request }) {
  ...
  const data = await Service.fetchGraduateLogs(search, page, limit);
  return { data, search, page };
}

function PgAISGraduateLogs({}: Props) {
          ...
          {view == "card" && (
            ...
            >
              {data?.data?.map((row: any) => (
                <GraduateLogCardItem key={row.id} data={row} />
              ))}
              {!data?.data?.length && (
...
export default PgAISGraduateLogs;
