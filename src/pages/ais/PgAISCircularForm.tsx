...
export async function action({ request, params }) {
  ...
  let resp;
  if (id != 0) resp = await Service.updateNotice(id, data);
  else resp = await Service.postNotice(data);
...
export async function loader({ params }) {
  ...
  const id = params?.noticeId || 0;
  if (id != 0) data = await Service.fetchNotice(id);
  return { data };
}
...
function PgAISCircularForm({}: Props) {
  ...
  return (
    <main className="md:pl-10 p-2 md:p-6 space-y-4 md:space-y-10">
      <SubPageTitle
        title={`${data?.id ? "Edit" : "Create"} Circular`}
        page="Circular"
      />
          ...
          <div className="p-3 md:py-6 md:pb-10 md:px-6 border rounded-lg md:rounded-xl bg-white space-y-3 md:space-y-6">
            ...
            <div className="md:pl-6 space-y-4">
              ...
              <label className="flex flex-col space-y-2">
                ...
                <select
                  arial-label="receiver"
                  name="receiver"
                  defaultValue={Number(data?.receiver)}
                  required
                  className="focus:ring-0 border focus:border-slate-300  border-primary-dark/10 bg-primary-dark/5 text-sm md:text-base text-gray-500 rounded-md"
                >
          ...
          <div className="p-3 md:py-6 md:pb-10 md:px-6 w-full border rounded-lg md:rounded-xl bg-white space-y-3 md:space-y-6">
            <div className="md:pl-6 space-y-4">
              <label className="flex flex-col space-y-2">
                <span className="text-sm md:text-base text-gray-500 font-medium">
                  SMS Content &nbsp;&nbsp;&nbsp;
                  <small>(160 Characters per SMS)</small>
                </span>
                <textarea
                  arial-label="smsContent"
                  rows={12}
                  maxLength={320}
                  name="smsContent"
