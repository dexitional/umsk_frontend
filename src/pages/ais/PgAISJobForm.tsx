...
export async function action({ request, params }) {
  ...
  let resp;
  if (id != 0) resp = await Service.updateJob(id, data);
  else resp = await Service.postJob(data);
...
export async function loader({ params }) {
  ...
  const id = params?.jobId || 0;
  if (id != 0) data = await Service.fetchJob(id);
  return { data };
}
...
function PgAISJobForm({}: Props) {
  ...
  return (
    <main className="md:pl-10 p-2 md:p-6 space-y-4 md:space-y-10">
      <SubPageTitle
        title={`${data?.id ? "Edit" : "Create"} Job Designation`}
        page="Jobs"
      />
        ...
        <section className="flex md:space-x-6">
          <div className="flex-1 flex flex-col space-y-1 md:space-y-3">
            <h1 className="text-lg md:text-2xl tracking-wide font-semibold text-primary/70">
              {data?.id ? "Edit" : "Create"} Job Designation
            </h1>
          ...
          <div className="p-3 md:py-6 md:pb-10 md:px-6 w-full border rounded-lg md:rounded-xl bg-white space-y-3 md:space-y-6">
            <div className="md:pl-6 space-y-4">
              <label className="flex flex-col space-y-2">
                ...
                <select
                  arial-label="staffCategory"
                  name="staffCategory"
                  defaultValue={data?.staffCategory}
                  required
                  className="focus:ring-0 border focus:border-slate-300  border-primary-dark/10 bg-primary-dark/5 text-sm md:text-base text-gray-500 rounded-md"
                >
                  <option selected disabled>
