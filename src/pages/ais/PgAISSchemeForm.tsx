...
export async function action({ request, params }) {
  ...
  data.scoreRange = JSON.parse(data.scoreRange);
  data.status = data.status == 1;
  let resp;
  if (id != 0) resp = await Service.updateScheme(id, data);
  else resp = await Service.postScheme(data);
...
export async function loader({ params }) {
  ...
  const id = params?.schemeId || 0;
  if (id != 0) data = await Service.fetchScheme(id);
  return { data };
}
...
function PgAISSchemeForm({}: Props) {
  ...
  return (
    <main className="md:pl-10 p-2 md:p-6 space-y-4 md:space-y-10">
      <SubPageTitle
        title={`${data?.id ? "Edit" : "Create"} Scheme`}
        page="Schemes"
      />
          ...
          <div className="p-3 md:py-6 md:pb-10 md:px-6 border rounded-lg md:rounded-xl bg-white space-y-3 md:space-y-6">
            ...
            <div className="md:pl-6 space-y-4">
              ...
              <label className="flex flex-col space-y-2">
                ...
                <textarea
                  arial-label="gradeMeta"
                  name="gradeMeta"
                  defaultValue={JSON.stringify(data?.gradeMeta)}
                  rows={18}
                  className="focus:ring-0 border focus:border-slate-300 border-primary-dark/10 bg-primary-dark/5 text-sm md:text-base text-gray-500 rounded-md tracking-widest"
                ></textarea>
              </label>
            </div>
          </div>
          ...
          <div className="p-3 md:py-6 md:pb-10 md:px-6 w-full border rounded-lg md:rounded-xl bg-white space-y-3 md:space-y-6">
            <div className="md:pl-6 space-y-4">
              <label className="flex flex-col space-y-2">
                ...
                <textarea
                  arial-label="classMeta"
                  name="classMeta"
                  defaultValue={JSON.stringify(data?.classMeta)}
                  rows={12}
                  className="focus:ring-0 border focus:border-slate-300 border-primary-dark/10 bg-primary-dark/5 text-sm md:text-base text-gray-500 rounded-md tracking-widest"
                ></textarea>
              </label>
              <label className="flex flex-col space-y-2">
                ...
                <textarea
                  arial-label="scoreRange"
                  name="scoreRange"
                  defaultValue={JSON.stringify(data?.scoreRange)}
                  rows={5}
                  className="focus:ring-0 border focus:border-slate-300 border-primary-dark/10 bg-primary-dark/5 text-sm md:text-base text-gray-500 rounded-md tracking-widest"
                ></textarea>
              </label>
