...
export async function action({ request, params }) {
  ...
  let resp;
  if (id != 0) resp = await Service.updateGraduateSession(id, data);
  else resp = await Service.postGraduateSession(data);
...
export async function loader({ params }) {
  ...
  const id = params?.sessionId || 0;
  if (id != 0) data = await Service.fetchGraduateSession(id);
  return { data };
}
