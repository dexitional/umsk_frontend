...
export async function action({ request, params }) {
  ...
  let resp;
  if (id != 0) resp = await Service.updatePersonel(id, data);
  else resp = await Service.postPersonel(data);
...
export async function loader({ params }) {
  let data = { id: 0 };
  const id = params?.personelId || 0;
  if (id != 0) data = await Service.fetchPersonel(id);
  return { data };
}
...
function PgDricPersonnelForm({}: Props) {
  ...
  return (
    <main className="md:pl-10 p-2 md:p-6 space-y-4 md:space-y-10">
      <SubPageTitle
        title={`${data?.id ? "Edit" : "Create"} Personnel`}
        page="Personnels"
      />
