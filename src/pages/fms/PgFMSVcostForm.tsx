...
export async function action({ request, params }) {
  ...
  let resp;
  if (id != 0) resp = await Service.updateVcost(id, data);
  else resp = await Service.postVcost(data);
...
export async function loader({ params }) {
  ...
  const id = params?.costId || 0;
  if (id != 0) data = await Service.fetchVcost(id);
...
function PgFMSScostForm({}: Props) {
  ...
  return (
    <main className="md:pl-10 p-2 md:p-6 space-y-4 md:space-y-10">
      <SubPageTitle
        title={`${data?.id ? "Edit" : "Create"} Price`}
        page="Voucher Prices"
      />
            ...
            <div className="md:pl-6 space-y-4">
              ...
              <label className="flex flex-col space-y-2">
                ...
                <select
                  ...
                  name="sellType"
                  defaultValue={Number(data?.sellType)}
                  className="focus:ring-0 border focus:border-slate-300  border-primary-dark/10 bg-primary-dark/5 text-sm md:text-base text-gray-500 rounded-md"
                >
                  <option selected disabled>
                    -- Choose --
