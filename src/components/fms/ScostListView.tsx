...
import ScostListItem from './ScostListItem'

type Props = {
  data: any
}

function ScostListView({ data }: Props) {
  return (
    <div className="py-6 grid grid-cols-1 gap-y-4 border bg-slate-50/50 rounded-xl">
      ...
      <div className="grid grid-cols-1 gap-y-4 text-xs text-slate-600 font-medium">
          { data && data?.map((row:any) => (<ScostListItem key={row.id} data={row} />))}
