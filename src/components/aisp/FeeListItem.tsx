    ...
    <div className="px-4 md:px-6 py-4 grid md:grid-cols-6 gap-3 md:gap-4 md:items-center border-b border-slate-50 last:border-0 hover:bg-slate-50/70 transition-colors">
        <div className="md:col-span-2 flex flex-col space-y-1.5">
           ...
           <div className="flex items-center space-x-3 min-w-0">
            <div className={`h-8 w-8 shrink-0 rounded-lg flex items-center justify-center ${isPayment ? 'bg-primary/10' : 'bg-primary-accent/10'}`}>
              <MdOutlinePayments className={`h-4 w-4 ${isPayment ? 'text-primary' : 'text-primary-accent'}`} />
            </div>
        ...
        <div className="flex flex-col space-y-1.5">
          ...
          <span className={`inline-flex w-fit items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${isPayment ? 'bg-primary/10 text-primary' : 'bg-primary-accent/10 text-primary-accent'}`}>
            <span className={`h-1.5 w-1.5 rounded-full ${isPayment ? 'bg-primary' : 'bg-primary-accent'}`} />
