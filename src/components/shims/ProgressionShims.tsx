...
function ProgressionShims({}: Props) {
  const [ view, setView ] = useState('card')
  const lm = useUserStore.getState().limit;
  const pglimit  = lm?.progressions;
  if(!pglimit) useUserStore.setState({ limit: { ...lm, 'progressions': 9 }});
  const limit = (pglimit || 9);
  

  return (
    <div className="md:pl-10 p-4 md:p-6 space-y-4 md:space-y-10">
      <PageTitleShims title="Progressions" createtext="" createlink="create"  />
      <div className="">
         { view == 'card' && (
            <div className="grid md:grid-cols-3 gap-3 md:gap-6">
                 {/* Card Shim */}
                { Array(limit).fill(limit).map((_,i) => (
                <div key={i} className="p-4 md:p-6 min-h-max border border-slate-200 rounded-xl bg-slate-50/50 space-y-8 animate-pulse delay-75">
...
export default ProgressionShims
