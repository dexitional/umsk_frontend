...
function GraduateSessionShims({}: Props) {
  const [ view, setView ] = useState('card')
  const lm = useUserStore.getState().limit;
  const pglimit  = lm?.graduatesessions;
  if(!pglimit) useUserStore.setState({ limit: { ...lm, 'graduatesessions': 9 }});
  const limit = (pglimit || 9);
  

  return (
    <div className="md:pl-10 p-4 md:p-6 space-y-4 md:space-y-10">
      <PageTitleShims title="Graduate Sessions" createtext="New" createlink="create"  />
      <div className="">
         { view == 'card' && (
...
export default GraduateSessionShims
