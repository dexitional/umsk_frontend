...
import ResitSessionListItem from './ResitSessionListItem'

type Props = {
  data: any
}

function ResitSessionListView({ data }: Props) {
  return (
    <div className="py-6 grid grid-cols-1 gap-y-4 border bg-slate-50/50 rounded-xl">
      ...
      <div className="grid grid-cols-1 gap-y-4 text-xs text-slate-600 font-noto font-medium">
          { data && data?.map((row:any) => (<ResitSessionListItem key={row.id} data={row} />))}
...
export default ResitSessionListView