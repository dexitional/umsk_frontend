...
function ProgressionCardItem({ data }: Props) {
  ...
  <div className="p-4 md:p-6 min-h-max border border-primary/20 rounded-xl bg-slate-50/50 hover:bg-slate-100 space-y-4 md:group font-roboto">
    <h2 className="text-base md:text-lg font-semibold text-gray-500 uppercase">{data?.student?.id}</h2>
    ...
    <div className="flex flex-col space-y-1">
        <div className="px-3 py-2 opacity-80 md:opacity-100 md:hidden flex rounded-md border bg-blue-50/30 items-center md:justify-between space-x-2 md:group">
          <div className="flex group-hover:hidden items-center justify-center space-x-3 text-center">
              ...
              <span className="font-semibold text-base text-primary/60">{Math.ceil(data?.student?.semesterNum/2) == 0 ? "COMPLETED": Math.ceil(data?.student?.semesterNum/2) * 100}</span>
          </div>
        </div>
        <div className="px-3 py-2 opacity-80 md:opacity-100 hidden md:flex rounded-md border bg-white items-center space-x-2 group">
         <div className="hidden md:flex items-center justify-center space-x-3 text-center">
              ...
              <span className="font-semibold text-base text-primary/60">{Math.ceil(data?.student?.semesterNum/2) == 0 ? "": Math.ceil(data?.student?.semesterNum/2) * 100}</span>
          </div>
        </div>
    </div>
